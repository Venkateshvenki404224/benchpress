<template>
  <div class="animate-fade-up">
    ...
    <div v-else class="space-y-3">
      <router-link v-for="bench in benches" :key="bench.name" :to="`/dashboard/bench/${bench.name}`" class="block">
        <GlassCard class="hover:-translate-y-0.5 transition-transform">
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="text-bp-text font-medium">{{ bench.bench_name || bench.name }}</h3>
            ...
            <div class="flex items-center gap-6">
              ...
              <div class="w-24 hidden sm:block">
                <StatBar label="RAM" :value="bench.memory_usage || 0" color="cyan" />
              </div>
              <span class="text-xs font-medium px-2 py-1 rounded" :class="statusBadgeClass(bench.status)">
                {{ bench.status }}
              </span>
            </div>
          </div>
