  ...
  <div class="animate-fade-up">
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-bp-muted text-sm mt-1">Pre-configured bench templates ready to deploy</p>
      </div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <GlassCard v-for="lab in labs" :key="lab.name" :shimmer="lab.status === 'Ready'" class="cursor-pointer hover:-translate-y-0.5 transition-transform">
        <div class="flex items-start justify-between mb-4">
          <div>
            <h3 class="text-lg font-semibold text-bp-text">{{ lab.title || lab.lab_id }}</h3>
            <p class="text-bp-muted text-sm">{{ lab.frappe_version }}</p>
          </div>
          <span class="px-2 py-0.5 rounded text-xs font-medium" :class="statusClass(lab.status)">
            {{ lab.status }}
          </span>
        </div>
        ...
        <div class="flex gap-2">
          <router-link :to="`/dashboard/deploy/${lab.name}`" class="btn-primary text-xs" v-if="lab.status === 'Ready'">
            Deploy
          </router-link>
          <button @click.stop="buildImage(lab.name)" class="btn-secondary text-xs" :disabled="lab.status === 'Building'">
