<template>
  <div class="animate-fade-up max-w-3xl mx-auto">
    <router-link to="/dashboard" class="text-bp-muted hover:text-bp-text text-sm mb-6 inline-flex items-center gap-1">
      ← Back to Labs
    </router-link>

    <h1 class="text-2xl font-bold text-bp-text mb-2">Deploy Bench</h1>
    <p class="text-bp-muted text-sm mb-8" v-if="lab">From <span class="text-bp-green">{{ lab.title || lab.lab_id }}</span> · {{ lab.frappe_version }}</p>
      ...
      <div class="space-y-4">
        ...
        <div v-if="lab?.apps?.length">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="app in lab.apps" :key="app.app_name" class="px-2 py-1 rounded text-xs bg-white/5 text-bp-muted">
              {{ app.app_name }}
            </span>
          </div>
        </div>
        <button @click="startDeploy" class="btn-primary w-full py-3 text-center" :disabled="!lab">
          Deploy Bench
        </button>
      </div>
    </GlassCard>
    ...
    <div v-if="deploying || deployDone">
      <GlassCard class="mb-4">
        <div class="flex items-center gap-3 mb-4">
          <StatusDot :status="deployDone ? (deployError ? 'error' : 'running') : 'deploying'" />
          <span class="font-medium text-bp-text">
            {{ deployDone ? (deployError ? 'Deploy Failed' : 'Deploy Complete') : 'Deploying...' }}
          </span>
        </div>
      ...
      <div v-if="deployDone && !deployError" class="flex gap-3">
        ...
        <router-link v-if="benchName" :to="`/dashboard/bench/${benchName}`" class="btn-secondary">Bench Detail</router-link>
      </div>
    </div>
  </div>
</template>
...
async function startDeploy() {
  ...
  logLines.value = [{ message: 'Starting deployment...', type: 'info', timestamp: new Date().toISOString() }];
  try {
