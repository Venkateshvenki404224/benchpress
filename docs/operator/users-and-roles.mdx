...
description: The two BenchPress roles, what each one may read and write, which screens are admin-only, and how ownership rather than a role decides who sees a bench.
---
...
1. Open `/app/user` in Desk **as a System Manager** and pick the person, then
   ...
   **A `BenchPress Admin` cannot do this.** That role grants no access to the
   `User` doctype, so `/app/user` answers `User <email> does not have doctype
   access via role permission for document User`. Granting roles is a
