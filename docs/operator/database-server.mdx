...
description: One MariaDB holds every bench site's database — where it lives, how BenchPress drives it, what drift detection watches, and the four actions on the record.
---
...
On this host the check reads `MariaDB responding at benchpress-mariadb, buffer
pool hit rate 99.97%, on the declared settings` — no drift. The 128 MB pool
