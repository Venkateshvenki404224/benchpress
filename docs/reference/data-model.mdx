---
title: Data model
description: All 20 BenchPress DocTypes with their fields, links, naming and the permission rule that scopes each one — plus why there is no Device DocType.
---
...
An admin sees everything. `Administrator`, `System Manager` and `BenchPress
Admin` all short-circuit every rule above.
