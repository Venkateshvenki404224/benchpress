...
frappe.ui.form.on("Lab", {
	refresh(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__("Build Image"), function () {
				frappe.confirm(
					__("This will build a Docker image for this lab. It may take 5-10 minutes. Proceed?"),
					function () {
						...
						frappe.call({
							...
							callback: function (r) {
								frappe.show_alert({
									message: __("Image build started. Check Build Log in the sidebar."),
									indicator: "blue",
								});
	...
	show_latest_build_status(frm) {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				let log = r.message[0];
				let color = log.log_type === "success" ? "green" : log.log_type === "error" ? "red" : "orange";
				let label = log.log_type === "success" ? "Build Passed" : log.log_type === "error" ? "Build Failed" : "Building...";

				if (frm.fields_dict.build_log_html) {
					frm.fields_dict.build_log_html.$wrapper.html(
						'<div style="padding: 12px;">'
						+ '<a href="/app/build-log/' + log.name + '" style="color: var(--text-color); text-decoration: none;">'
						...
						+ '<span class="indicator-pill ' + color + '">' + label + '</span>'
						+ '<span style="color: var(--text-muted); font-size: 12px;">Latest build — ' + frappe.datetime.prettyDate(log.creation) + '</span>'
