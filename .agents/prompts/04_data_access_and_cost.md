...
- Claude passes an explicit `limit` to every `get_all` / `get_list`, because a list that is short
  in development is the same list that has 40 000 deploy log rows in production.
...
- Claude requests only the fields it renders. A `fields=[...]` list is cheaper than a `get_doc`
...
- Claude does not call `frappe.get_doc`, `frappe.get_cached_doc` or `frappe.db.get_value` inside a
  loop over rows; nine sites in the tree still do, including `docker_events.py:171` and
...
- A Docker call inside a loop is the expensive case: `hooks.py:255` records that the stats sweep
  ...
  cron slot.
- Claude batches a scheduled sweep onto the worker that already holds what it needs —
...
- The SPA has zero `setInterval` uses and pairs all three `setTimeout` calls with a
  `clearTimeout`; Claude keeps that pairing, since a timer surviving unmount fires against a dead
  component.
- There is no `AbortController` anywhere in `frontend/src`, so a request from an abandoned screen
  still completes. Claude does not add a new long-running fetch without cancelling it, and the
...
- Build target is `es2015` and `chunkSizeWarningLimit` is 1500 (`frontend/vite.config.js:25-28`);
  Claude does not raise the limit to silence a warning — the warning is the signal.
...
- `optimizeDeps.include` at `vite.config.js:36` pre-bundles `feather-icons` and
  `highlight.js/lib/core`, neither of which `frontend/src` imports — Claude removes an entry there
  when removing its last import rather than leaving the pre-bundle behind.
...
- A stylesheet's content hash changes on every build even when the file did not, because Frappe's
  ...
  sourcemap. Claude does not chase that diff; scripts are content-stable and stylesheets are not.
