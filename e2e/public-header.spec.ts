...
// The header is one include across all six public routes, so a width that breaks it breaks
// every page at once. 320px is the floor: below the last breakpoint, nothing shrinks further.
const ROUTES = ["/", "/landing", "/signup", "/login", "/about", "/contact"];
const WIDTHS = [320, 360, 390, 480, 560, 768, 900, 1024, 1280];
...
test.describe("Public header responsiveness", () => {
  for (const route of ROUTES) {
    test(`${route} header stays inside the viewport at every width`, async ({ page }) => {
      for (const width of WIDTHS) {
        ...
        const overflow = await page.evaluate(() => {
          const header = document.querySelector<HTMLElement>(".bp-header");
          if (!header) return "no .bp-header on page";
        ...
        expect(overflow, `${route} at ${width}px`).toBe("");
      }
    });
  }
  ...
  // The wordmark shrinks through two breakpoints. Both rules once lost to a more specific
  // per-variant base rule, which a media query cannot outweigh.
  test("the wordmark shrinks on both header variants", async ({ page }) => {
    ...
    expect(heights.dock).toEqual(["22px", "24px", "30px"]);
    expect(heights.bar).toEqual(["22px", "24px", "26px"]);
  });
