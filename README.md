...
BenchPress installs into a Frappe bench you already run. There is no `git clone` path.

```bash
# 1. From your bench root. vpn_management is a required_app and bench will not fetch it for you.
...
# 2. Build the single-page app, from this repo's root (apps/benchpress).
cd frontend && yarn install && yarn build

# 3. The everyday loop, all from this repo's root.
cd frontend && yarn dev          # SPA with hot reload
cd frontend && yarn test:run     # 153 SPA unit tests
cd e2e && npx playwright test    # has its own config; the repo root resolves nothing
npm run docs:build && npm run docs:lint && npm run docs:score && npm run docs:links

# 4. What CI runs. Never name the `frontend` site here — it carries no allow_tests,
#    and an interrupted run leaves its scheduler off, which silently stops all mail.
bench --site bp_test_site run-tests --app benchpress
uvx pre-commit@4.3.0 run --all-files
```

Then open the app at `/frontend` on your site. `setup.sh` covers the host side — the Docker
group, the shared MariaDB and Redis containers, and IP forwarding. Full detail is in
[Install](docs/operator/install.mdx) and [Prerequisites](docs/operator/prerequisites.mdx).
...
| Path | Contents |
|---|---|
| [`benchpress/`](benchpress/) | The Frappe app — 164 non-test Python modules and 22 DocTypes |
| [`benchpress/api.py`](benchpress/api.py) | The whitelisted API. Every method checks permissions for itself |
| [`benchpress/docker_manager.py`](benchpress/docker_manager.py) | Image builds, container creation, resource limits, labels and health |
| [`benchpress/hooks.py`](benchpress/hooks.py) | Document events, permission rules and the 11 scheduled jobs |
| [`benchpress/www/`](benchpress/www/) | The public site — 11 templates, all behind the `benchpress_public_site` key |
| [`benchpress/lab-templates/`](benchpress/lab-templates/) | The lab image `Dockerfile` and the six scripts baked into every bench |
| [`benchpress/config/`](benchpress/config/) | Compose file for the shared MariaDB and Redis containers |
| [`frontend/`](frontend/) | The Vue 3 SPA — 59 components and 16 data modules, built by Vite |
| [`docs/`](docs/) | Documentation source: 40 `.mdx` pages. The only hand-edited docs tree |
| [`docs-site/`](docs-site/) · [`docs-bundle/`](docs-bundle/) | Generated by leadtype. Never hand-edited |
| [`e2e/`](e2e/) | Playwright suite — 10 specs and 8 page objects, with its own config |
| [`.agents/prompts/`](.agents/prompts/) | The six working prompts: coding rules measured on this repository |
| [`scripts/`](scripts/) | Repository tooling, including the generated-docs link checker |
| [`internal/`](internal/) | Working notes and the design handoff. Not published |
...
[CLAUDE.md](CLAUDE.md) is the orientation file: the environment, the everyday commands, the
repo-wide invariants, and the handful of things that bite a newcomer.

The coding rules live in [.agents/prompts/](.agents/prompts/) as six standing prompts. Every
number in them was measured on this repository, and every `path:line` was verified. Read
`00_app_context.md` first, then the one that matches the task.
...
Install the hooks once. They are the same checks the `Frappe Linter` job runs.
...
Never run `yarn lint`. That script is biome, a style this repo has never used, and it rewrites
every file under `frontend/`. Nothing in this repo typechecks either: the SPA is plain
JavaScript, and there is no tracked `tsconfig.json`.

| Check | What it runs | Scope | Blocks a merge |
|---|---|---|---|
| Server (`ci.yml`) | `bench run-tests --app benchpress` on `test_site`, then a page-load check | Backend paths on a pull request | No |
| Frontend (`ci.yml`) | `yarn test:run`, then `yarn build` | `frontend/**` | No |
| Docs (`ci.yml`) | build, lint, score, dead-link check, generated-tree drift | Every push and pull request | No |
| Frappe Linter (`linter.yml`) | pre-commit (ruff, prettier, eslint) and a semgrep diff scan | Pull requests only | No |
| Vulnerable Dependency Check (`linter.yml`) | `pip-audit` over the manifest | Every push and pull request | No |

Every row reads "No" because `version-16` carries no branch-protection rule and no ruleset today.
A red run can still merge, so read each job before you do.
