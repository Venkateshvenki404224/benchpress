...
Four commands build and gate these pages. Read the exit code, never the last
line of output.

```bash
npm run docs:build && npm run docs:lint && npm run docs:score; echo "exit=$?"
```
...
Two traps are already handled by the scripts, and both cost a debugging session
to find.
