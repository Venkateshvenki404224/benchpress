...
|`scripts/golden_drill.py`|what a golden restore costs against a cold deploy, and how long a deploy takes to a first login|
...
`--concurrent N` starts N deploys at once, one for each drill user.
`--probe-login` times each deploy to the first login on its new site. It reaches
the site through the host's own Traefik at `BENCHPRESS_EDGE`, not through the CDN.

`scripts/restore_probe.sh` is a shell script, not a drill. It times one lab's
golden dump restored under four MariaDB configurations, each in a throwaway
`bp-restore-probe` container. It never touches `benchpress-mariadb`.
`--list` prints the four configurations. See
