...
Three endpoints answer a guest. They are listed separately in
[The three guest endpoints](#the-three-guest-endpoints), and each is rate limited.
...
## The three guest endpoints

Three functions carry `allow_guest=True`. They are the whole surface an
unauthenticated caller can reach. All three take POST only, and all three refuse
unless `benchpress_public_site` is set — see
...
`contact.submit` records the message and answers the same way whether or not the
mail went out.
